...
#pragma warning restore
        ...
        public void StringExtensionTest()
        {
            ...
            WriteLine("每隔4个字符插入一个逗号,输出结果为:{0}", "独立国家哈桑领导讲话稿按时鉴定会gals的机会格拉斯的".Spacing(",", 3));
            WriteLine();
            WriteLine("字符串的全角半角转换:测试字符串:'我是ABC,你是谁?'");
            string sbc = "我是ABC,你是谁?".ToSBC();
            WriteLine("转化为全角:{0}", sbc);
            WriteLine("转化为半角:{0}", sbc.ToDBC());
            WriteLine();
            WriteLine("测试字符串是否为数字函数:测试字符串为:'3634635'和'767sf'");
            WriteLine("3634635:{0}","3634635".IsNumber());
            WriteLine("767sf:{0}", "767sf".IsNumber());
            WriteLine();
            WriteLine(@"验证某个字符串是否符合某个正则表达式:测试方式为电子邮件:test@qq.com和www.qq.com,正则为:'[\w]+(\.[\w]+)*@[\w]+(\.[\w])+'");
            WriteLine("test@qq.com的结果:{0}", "test@qq.com".QuickValidate(@"[\w]+(\.[\w]+)*@[\w]+(\.[\w])+"));
            WriteLine("www.qq.com的结果:{0}", "www.qq.com".QuickValidate(@"[\w]+(\.[\w]+)*@[\w]+(\.[\w])+"));
            WriteLine();
            WriteLine("--------------------------------------------------------------------------------------------------------------------");
